...
$.widget('calvinUI.masker', {
    ...
    _create: function () {
        ...
        var isWindow = elem == window || elem == document
			|| !elem.tagName || (/^(?:body|html)$/i).test(elem.tagName);
        if (isWindow) {
        ...
        this.ie6 = $.browser.msie && parseInt($.browser.version) === 6, this.boxModel = $.boxModel;
        this.CreateMask();
        this.SetMaskStyle();

    },
    CreateMask: function () {
        var opts = this.options, z_index = opts.baseZ;
        //构造iframe层 遮住select
        var iframeLayer;
